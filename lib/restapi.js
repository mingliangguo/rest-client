...
module.exports = class RestAPI {
    ...
    retry(options, wait_time = DEFAULT_RETRY_WAIT_TIME) {
        if (isNaN(wait_time )) {
            logger.log('warn', `NaN[${wait_time}] is passed in as retry time!!`);
        ...
        return utils
            ...
            .catch(err => {
                return this.handleResponse(options, undefined, err);
            });
    }
    ...
    handleResponse(options, res, err) {
        ...
        if (this.shouldRetry(options, res)) {
            ...
            return (
                (this.getResponseHandler &&
                    this.getResponseHandler(options, res, err)) ||
                res
            );
        }
    }
